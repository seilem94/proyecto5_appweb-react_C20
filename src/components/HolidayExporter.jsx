...
function HolidayExporter({ holidays, countryName, year }) {
  ...
  const exportToJSON = () => {
    const data = {
      ...
      festivos: holidays.map(h => ({
        ...
        diaSemana: new Date(h.date).toLocaleDateString('es-ES', { weekday: 'long' })
      }))
    };

    const jsonContent = JSON.stringify(data, null, 2);
    downloadFile(jsonContent, `festivos_${countryName}_${year}.json`, 'application/json');
  };
  ...
  const exportToICal = () => {
    ...
    downloadFile(icalContent, `festivos_${countryName}_${year}.ics`, 'text/calendar');
  };
    ...
    >
      ...
      >
        <Typography variant="caption" color="text.secondary">
          💡 El archivo iCal (.ics) puede importarse directamente en Google Calendar, Outlook, Apple Calendar y otras aplicaciones de calendario.
        </Typography>
      </Box>
    </Paper>
  );
